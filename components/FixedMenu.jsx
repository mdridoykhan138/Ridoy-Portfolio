    ...
    <div className="fixed w-full h-[400px] z-50 bottom-[4rem] flex justify-center pointer-events-none bg-transparent">
      ...
      <AnimatePresence>
        {showMenu && showMenuButton && (
          ...
          <div className="bg-white w-full h-full shadow-custom max-w-[1170px] mx-auto py-12 xl:py-12 px-12 xl:px-32 flex items-center gap-12">
            {/* navber menu */}
            <Nav containerStyles="md:border-r border-secondary/20 md:pr-12 w-full md:w-auto text-center md:text-left" listStyles="flex flex-col justify-center gap-4" linkStyles="font-primary text-4xl text-primary cursor-pointer" spy={true}/>
            {/* info */}
      ...
      {isMobile ? (
        <div className="fixed z-50 bottom-16">
          <button onClick={() => setShowMenu(!showMenu)} className="bg-accent shadow-custom w-[54] h-[54] rounded-lg cursor-pointer flex items-center justify-center select-none pointer-events-auto">
        ...
        <AnimatePresence>
          {showMenuButton && (
            ...
            >
              <button onClick={() => setShowMenu(!showMenu)} className="bg-accent shadow-custom w-[54] h-[54] rounded-lg cursor-pointer flex items-center justify-center select-none"><CgMenuGridR className="text-4xl text-white"/></button>
            </motion.div>
          )}
        </AnimatePresence>
      )}
